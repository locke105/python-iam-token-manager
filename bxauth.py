...
class TokenManager(object):
    ...
    def _refresh_token(self):
        token_resp = auth(refresh_token=self._token_info.get('refresh_token'), iam_endpoint=self.iam_endpoint)
        if isinstance(token_resp, dict):
            self._token_info = token_resp
        else:
    ...
    def is_refresh_token_expired(self):
        # no idea how long these last, but some other code suggested up to 30 days,
...
def auth(username=None, password=None, apikey=None, refresh_token=None, iam_endpoint=None):
    if not iam_endpoint:
...
def main():
    ...
    possible_keys = [os.environ.get('BLUEMIX_API_KEY'), os.environ.get('IBMCLOUD_API_KEY')]
